...
/*
** WiFi credentials live in their own file so the confStruct stays untouched:
** no config version bump, no re-pairing, and Luddi96's config tool keeps
** working. Same idea as batconf.txt above.
** Format is three lines: SSID, WiFi password, maintenance password.
*/
bool readWifiFromSPIFFS()
{
    if (!SPIFFS.exists(WIFI_FILE_PATH)) {
        Serial.println("No wifi conf in SPIFFS");
        return false;
    }

    File file = SPIFFS.open(WIFI_FILE_PATH, FILE_READ);
    if (!file) {
    ...
    wifi_otapass = file.readStringUntil('\n');
    file.close();
    ...
    if (wifi_ssid.length() == 0 || wifi_otapass.length() == 0) {
        Serial.println("Wifi conf incomplete, need SSID and maintenance password");
        return false;
    }

    Serial.print("Wifi conf read, SSID: ");
    Serial.println(wifi_ssid);
    return true;
}

void saveWifiToSPIFFS(String ssid, String pass, String otapass)
{
    File file = SPIFFS.open(WIFI_FILE_PATH, FILE_WRITE);
    if (!file) {
    ...
    file.println(ssid);
    file.println(pass);
    file.println(otapass);
    file.close();
    Serial.println("Wifi conf saved to SPIFFS");
}

void deleteWifiFromSPIFFS()
{
    if (SPIFFS.remove(WIFI_FILE_PATH)) {
        Serial.println("Wifi file deleted successfully");
    } else {
        Serial.println("Failed to delete wifi file");
    }
}
